      ...
      <div className="max-w-3xl w-full mx-auto space-y-2 mb-20">
        {logs.map((log, i) => (
          <div key={i} className="text-[#00f0ff] mono text-sm sm:text-base md:text-lg animate-in fade-in slide-in-from-bottom-2 duration-300">
            <span className="opacity-50 mr-4">{`[${new Date().toISOString().split('T')[1].slice(0, 12)}]`}</span>
            <span className={i === bootSequence.length - 1 ? "text-[#ff003c] font-bold text-xl glitch-anim inline-block" : ""}>{log}</span>
          </div>
        ))}
        {!isBooted && (
