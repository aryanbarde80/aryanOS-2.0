...
export default function AchievementsNode() {
  ...
  return (
    <OSWindow title="ACHIEVEMENTS_&_AWARDS.LOG" icon={<Award size={16} className="text-[#fb923c] animate-pulse" />}>
      <div className="w-full relative">
        <div className="absolute left-[7px] top-0 bottom-0 w-[1px] bg-gradient-to-b from-[#fb923c] via-[#818cf8] to-transparent opacity-40"></div>
        <ul className="space-y-3 sm:space-y-4">
          {achievements.map((item, idx) => {
            const IconComponent = item.icon;
            return (
              <li key={idx} className="relative pl-6 sm:pl-8 group">
                <div className="absolute left-1 top-1.5 w-2 h-2 rounded-full bg-[#fb923c] shadow-[0_0_8px_rgba(255,170,68,0.8)] group-hover:scale-150 transition-transform"></div>
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between border-b border-[#818cf8]/10 pb-2 sm:pb-3 group-hover:border-[#fb923c]/40 transition-colors">
                  <div className="flex items-start gap-2 sm:gap-3 flex-1">
                    <IconComponent size={14} className="text-[#fb923c] mt-1 flex-shrink-0 hidden sm:block" />
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[#818cf8] mono text-xs sm:text-sm mt-0.5 group-hover:drop-shadow-[0_0_5px_rgba(0,240,255,0.4)] break-words">
                        {item.rank}
                      </p>
                    </div>
                  </div>
                  <span className="text-[10px] sm:text-xs mono text-gray-500 mt-2 sm:mt-0 ml-0 sm:ml-4 flex-shrink-0 group-hover:text-[#fb923c] transition-colors">
                    {item.date}
                  </span>
                </div>
              </li>
        ...
        <div className="mt-4 pt-3 border-t border-[#818cf8]/20 flex justify-between items-center flex-wrap gap-2">
          ...
          <div className="flex items-center gap-2">
            <Medal size={12} className="text-[#818cf8]" />
